...
bool WidgetMgr::init( ID3D11Device* device, ID3D11DeviceContext* context, Mouse* pMouse )
{
    mDevice = device;
    mCtx = context;
    mpMouse = pMouse;
    ...
    mpMouse->EventMove.hook(MouseEvent::SIGNAL_TYPE::slot_type(&WidgetMgr::onMouseMove, this, _1, _2));
...
void WidgetMgr::setMouse( Mouse* pMouse )
{
    mpMouse = pMouse;
}

void WidgetMgr::drawUI()
{
...
void WidgetMgr::batchBegin()
{
}

void WidgetMgr::batchEnd()
{
    clearGraphicsBuffers();
...
WidgetBase* WidgetMgr::createWidget( float posX, float poxY, int width, int height, std::wstring& labelText, std::wstring widgetType /*= L"Widget/Button"*/ )
{
...
void WidgetMgr::createGraphicsBuffers()
{
    // Create a dynamic VB 
    //
...
void WidgetMgr::clearGraphicsBuffers()
{
    safe_release(&mGraphicsVB);
    safe_release(&mGraphicsIB);
}

void WidgetMgr::updateGraphicsBuffers()
{
...
void WidgetMgr::createTextBuffers()
{
...
void WidgetMgr::clearTextBuffers()
{
    safe_release(&mTextVB);
    safe_release(&mTextIB);
}

void WidgetMgr::updateTextBuffers()
{
    D3D11_MAPPED_SUBRESOURCE mappedData;
