...
PCWSTR DemoBasic::getClassName() const
{
    return L"DemoBasic";
}

PCWSTR DemoBasic::getWindowCaption() const
{
    return L"Demo";
}
...
void DemoBasic::buildGird()
{

}

void DemoBasic::updateScene( float dt )
{
    //
    // Control the camera.
    //
...
void DemoBasic::onResize()
{
    RenderCore::onResize();
    mCam.SetLens(0.25f*MathHelper::Pi, aspectRatio(), 1.f, 1000.f);
}

void DemoBasic::onMouseDown( WPARAM btnState, int x, int y )
{
    SetCapture(mHwnd);
}

void DemoBasic::onMouseUp( WPARAM btnState, int x, int y )
{
    ReleaseCapture();
}

void DemoBasic::onMouseMove( WPARAM btnState, int x, int y )
{
